...
public class Converters {
     ...
     * @throws ApiException
     */
    public static <T> T convert(Class<T> clazz, Reader reader) throws IpayApiException {
        T rsp = null;
        ...
        try {
            ...
            boolean isResponseClazz = IpayResponse.class.isAssignableFrom(clazz);
            ...
            for (PropertyDescriptor pd : pds) {
                ...
                List<IpayFieldMethod> ipayFieldMethods = new ArrayList<IpayFieldMethod>();

                if (baseFields.contains(itemName) && isResponseClazz) {

                    Field field = IpayResponse.class.getDeclaredField(itemName);

                    IpayFieldMethod ipayFieldMethod = new IpayFieldMethod();
                    ipayFieldMethod.setField(field);

                    // writeMethod属于父类，则直接使用
                    if (writeMethod.getDeclaringClass().getName().contains("IpayResponse")) {

                        ipayFieldMethod.setMethod(writeMethod);
                    } else {
                        // 否则从父类再取一次
                        writeMethod = tryGetSetMethod(IpayResponse.class, field,
                                writeMethod.getName());
                        if (writeMethod == null) {
                            continue;
                        }
                    ...
                    ipayFieldMethods.add(ipayFieldMethod);
                    ...
                    if (overideFields.contains(itemName)) {
                        ...
                        if (field != null) {
                            ...
                            ipayFieldMethod = new IpayFieldMethod();
                            ipayFieldMethod.setField(field);
                            ipayFieldMethod.setMethod(writeMethod);
                            ipayFieldMethods.add(ipayFieldMethod);
                        }
                ...
                // 迭代设置属性
                for (IpayFieldMethod ipayFieldMethod : ipayFieldMethods) {

                    Field field = ipayFieldMethod.getField();
                    Method method = ipayFieldMethod.getMethod();
                    ...
                    if (String.class.isAssignableFrom(typeClass)) {
                        ...
                        if (value instanceof String) {
                            ...
                            if (isCheckJsonType && value != null) {
                                throw new IpayApiException(itemName + " is not a String");
                            }
                            if (value != null) {
                                method.invoke(rsp, value.toString());
                            } else {
                        ...
                        if (value instanceof Long) {
                            ...
                            if (isCheckJsonType && value != null) {
                                throw new IpayApiException(itemName + " is not a Number(Long)");
                            }
                        ...
                        if (value instanceof Integer) {
                            ...
                            if (isCheckJsonType && value != null) {
                                throw new IpayApiException(itemName + " is not a Number(Integer)");
                            }
                            if (StringUtils.isNumeric(value)) {
                        ...
                        if (value instanceof Boolean) {
                            ...
                            if (isCheckJsonType && value != null) {
                                throw new IpayApiException(itemName + " is not a Boolean");
                            }
                            if (value != null) {
                        ...
                        if (value instanceof Double) {
                            ...
                            if (isCheckJsonType && value != null) {
                                throw new IpayApiException(itemName + " is not a Double");
                            }
                        }
                        ...
                        if (value instanceof Number) {
                            ...
                            if (isCheckJsonType && value != null) {
                                throw new IpayApiException(itemName + " is not a Number");
                            }
                        }
                    } else if (Date.class.isAssignableFrom(typeClass)) {
                        DateFormat format = new SimpleDateFormat(IpayConstants.DATE_TIME_FORMAT);
                        format.setTimeZone(TimeZone.getTimeZone(IpayConstants.DATE_TIMEZONE));
                        Object value = reader.getPrimitiveObject(itemName);
                        if (value instanceof String) {
